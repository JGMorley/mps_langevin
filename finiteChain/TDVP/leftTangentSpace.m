function V = leftTangentSpace(n, A, left, varargin)
    %% Finds left tangent space of MPS tensor A 
    %
    %% Inputs
    %  A: tensor
    %    MPS tensor
    ...
    if p.Results.method == 'null'
        ...
        elseif isequal(size(size(A)), [1 2])
            ...
            should_be_zeros = ncon({conj(A),sqrtm(left),V},...
                                   {[1 2],[3 1],[3 -1 2]});
            if ~isequal(round(should_be_zeros, 10),zeros([size(V,2) 1]))
            ...
            iden_nullSpace = round(ncon({V, conj(V)}, {[1 -1 2],[1 -2 2]}), 10);
